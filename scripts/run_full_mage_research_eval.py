...
DEFAULT_DETECTORS = ["roberta", "fast_detectgpt", "binoculars", "mage"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-root",
        default=f"outputs/eval_runs/full_mage_research_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}",
    )
    parser.add_argument("--env-file", default=".secrets/eval.env")
    parser.add_argument("--checkpoint-json", default=".secrets/m2_checkpoint.json")
