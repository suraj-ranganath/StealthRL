...
\section{Results}
Table~\ref{tab:main} summarizes detection evasion at 1\% FPR. StealthRL (M2) achieves 0\% TPR@1\%FPR across all three detectors, reducing average AUROC across detectors from 0.74 (no attack) to 0.27 while maintaining semantic similarity (0.896 E5). Figure~\ref{fig:results} visualizes the comprehensive performance: panel (c) shows consistent 0\% TPR across detectors at strict operating points, while panel (d) confirms 99.9\% attack success rate. Qualitative examples of paraphrased outputs are provided in Appendix~A.
