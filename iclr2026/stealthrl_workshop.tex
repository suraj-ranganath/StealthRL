...
An anonymized code package with training and evaluation scripts is included in supplementary material. Placeholder: \url{https://anonymous.4open.science/r/STEALTHRL}.
...
\end{equation}
where $R_{\text{det}}$ is the ensemble detector score (lower AI probability) and $R_{\text{sem}}$ is E5 embedding cosine similarity. We train with group-relative policy optimization (GRPO) and LoRA adapters on Qwen3-4B-Instruct (rank 32, $\alpha=32$, dropout 0.05). Training uses group size 8, batch size 16, and two epochs over 10,000 samples from MAGE train split plus 200 dev samples. The detector ensemble is RoBERTa (0.6) + Fast-DetectGPT (0.4).

\paragraph{Reproducibility.} Reward weights: $\alpha=1.0$, $\beta=0.1$. GRPO uses KL penalty 0.05 with frozen Qwen3-4B as reference. Inference: temperature 1.0, top-p 0.9, max tokens 512. Detectors: RoBERTa (roberta-large-openai-detector), Fast-DetectGPT (GPT-Neo-2.7B), Binoculars (gpt2-medium/large, held-out). Full hyperparameters in Appendix~\ref{sec:hyperparams}.
...
\paragraph{Dataset.} We evaluate on MAGE (Machine-generated Text Detection in the Wild) test split with 1,000 human and 1,000 AI samples, filtered to 100--500 token window. \cite{li2024magemachinegeneratedtextdetection}

\paragraph{Detectors.} Three families: RoBERTa OpenAI (fine-tuned classifier), Fast-DetectGPT (curvature-based zero-shot), and Binoculars (paired-LM lightweight version, held-out during training). \cite{mitchell2023detectgptzeroshotmachinegeneratedtext,bao2024fastdetectgptefficientzeroshotdetection,hans2024spottingllmsbinocularszeroshot}

\paragraph{Baselines.} M0 (no attack), M1 (simple paraphrase via Qwen3:4B), M2 (StealthRL-v1), M3 (detector-guided selection), M5 (homoglyph substitution via SilverSpeak). \cite{cheng2025adversarialparaphrasinguniversalattack,creo2025silverspeakevadingaigeneratedtext}

\paragraph{Metrics.} We calibrate 1\% FPR thresholds per detector on human texts and report TPR@1\%FPR (lower better for attacker), ASR = 1 - TPR, AUROC, and E5 semantic similarity.

\section{Results}
Table~\ref{tab:main} summarizes detection evasion at 1\% FPR. StealthRL (M2) achieves 0\% TPR@1\%FPR across all three detectors, reducing mean AUROC from 0.74 (no attack) to 0.27 while maintaining semantic similarity (0.896 E5). Figure~\ref{fig:results} visualizes the comprehensive performance: panel (c) shows consistent 0\% TPR across detectors at strict operating points, while panel (d) confirms 99.9\% attack success rate.
